...
- **App.tsx**: Main app component with HashRouter setup and route configuration
- **pages/**: Route components (Home, TermsOfService, PrivacyPolicy, SpecifiedCommercialTransactions)
- **components/**: Reusable UI components (Hero, About, Services, Contact, etc.)
- **index.tsx**: React app entry point

### Routing Configuration

The app uses HashRouter for static hosting compatibility with these routes:
- `/` → Home page (combines all main sections)
...
- Tailwind CSS loaded via CDN with custom theme configuration in index.html
...
- `GEMINI_API_KEY`: Set in .env.local (referenced in vite.config.ts but not currently used in components)

### Import Map Configuration

Uses import maps in index.html for CDN-based React dependencies (aistudiocdn.com), enabling development without node_modules for React ecosystem packages.

## Development Notes

- The Contact component currently shows a placeholder alert - no backend integration
- Phone number and email are placeholder values (03-XXXX-XXXX, info@sharoushi-sample.jp)
- Static HTML files for legal pages were previously created but current architecture uses React components
- No test framework currently configured
- No linting tools configured in package.json
